...
//			{name: "log", content: $L("log"), onclick: "logClicked"},
			...
			{name: "Circle", className: "circle", onmousedown: "mydown", components: [
				...
				{name: "number1", content: $L("100"), className: "number n1"},
	...
	create: function() {
		...
		rotate: function (degrees)
		{
			...
			range = (360 - finite.sdeg) + finite.edeg;
			
			this.setPositionByDegree(this.$.number0, finite.sdeg);
			this.setPositionByDegree(this.$.number1, finite.edeg);

			currentRotation = degrees;

			if (currentRotation <= 180 && currentRotation > finite.edeg) {
				currentRotation = finite.edeg;
				this.dragFlag = false;
				this.downFlag = false;
				this.ballFlag = false;
			}
				
			if (currentRotation > 180 && currentRotation < finite.sdeg) {
				currentRotation = finite.sdeg;
				this.dragFlag = false;
				this.downFlag = false;
				this.ballFlag = false;
			}
		...
		degreeToNum: function (degrees) {	
			if (degrees < 180) deg = (360 - finite.sdeg) + degrees;
			if (degrees > 180) deg = (degrees - finite.sdeg);
			//= Math.round(/slotWidth);
			this.$.text.setContent(Math.round(deg/2.7));
		},
